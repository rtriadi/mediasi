...
<div class="flex flex-wrap items-center justify-between gap-4 mb-6">
    <div>
        ...
        <p class="text-xs text-slate-500 mt-1">Total: <strong><?= $total ?></strong> kategori jenis perkara yang dapat dimediasi</p>
    </div>
    <a href="<?= site_url('admin/master_jenis_perkara/tambah') ?>" id="btn-tambah-jp"
        ...
        <span>Tambah Jenis Perkara</span>
    </a>
</div>

<!-- Filter & Search Bar -->
<div class="bg-white rounded-2xl border border-slate-200/80 p-4 mb-6 shadow-sm">
    <form id="form-filter" method="GET" action="<?= site_url('admin/master_jenis_perkara') ?>" class="flex flex-wrap gap-4 items-end justify-between">
        <div class="flex flex-wrap gap-4 items-end w-full sm:w-auto">
            ...
            <div class="w-full sm:w-72">
                <label class="block text-xs font-bold text-slate-500 uppercase tracking-wider mb-1.5 whitespace-nowrap">Cari Jenis Perkara</label>
                <div class="relative">
        ...
        <a href="<?= site_url('admin/master_jenis_perkara') ?>" class="text-xs text-rose-600 hover:text-rose-800 font-bold py-2 px-3 bg-rose-50 rounded-xl border border-rose-200 flex items-center gap-1">
        ...
        <table class="w-full text-xs">
            <thead class="bg-slate-100/80 border-b border-slate-200 text-slate-600">
                <tr>
                    ...
                    <th class="text-left px-4 py-3.5 font-bold uppercase tracking-wider">Nama Jenis Perkara</th>
                    <th class="text-left px-4 py-3.5 font-bold uppercase tracking-wider">Keterangan / Deskripsi</th>
            ...
            <tbody class="divide-y divide-slate-100">
                ...
                <tr><td colspan="5" class="px-4 py-12 text-center text-slate-400 font-medium">Belum ada data jenis perkara.</td></tr>
                <?php else: ?>
                <?php $no = (isset($page) ? ($page-1)*10 : 0) + 1; foreach ($jenis_list as $jp): ?>
                <tr class="hover:bg-slate-50/80 transition-colors">
                    <td class="px-4 py-3.5 text-slate-400"><?= $no++ ?></td>
                    <td class="px-4 py-3.5 font-semibold text-slate-900"><?= htmlspecialchars($jp->nama) ?></td>
                    <td class="px-4 py-3.5 text-slate-600"><?= htmlspecialchars($jp->keterangan ?: '—') ?></td>
                    ...
                    <td class="px-4 py-3.5 text-right">
                        <div class="flex items-center justify-end gap-1.5">
                            <a href="<?= site_url("admin/master_jenis_perkara/edit/{$jp->id}") ?>"
                            ...
                            <a href="<?= site_url("admin/master_jenis_perkara/toggle_aktif/{$jp->id}") ?>"
                                class="text-xs <?= $jp->is_active ? 'text-amber-600 hover:text-amber-800 hover:bg-amber-50' : 'text-emerald-600 hover:text-emerald-800 hover:bg-emerald-50' ?> font-bold px-2.5 py-1 rounded-lg transition-colors">
                                <?= $jp->is_active ? 'Nonaktifkan' : 'Aktifkan' ?>
                            </a>
                            <a href="<?= site_url("admin/master_jenis_perkara/hapus/{$jp->id}") ?>"
                                onclick="return confirm('Yakin hapus jenis perkara <?= htmlspecialchars($jp->nama) ?>?')"
